...
NAV = """<a href='/'>📊 Dashboard</a><a href='/map'>🗺️ Map</a><a href='/chat'>💬 Chat</a><a href='/inventory'>🎒 Inventories</a><a href='/backups'>💾 Backups</a><a href='/world_upload'>🌍 World Upload</a><a href='/server_props'>⚙️ Server Props</a><a href='/gamerules'>🎮 Game Rules</a><a href='/stats'>📊 Stats</a><a href='/scheduler'>⏰ Scheduler</a><a href='/settings'>🛡️ Settings</a>"""
        ...
        class DashboardHandler(http.server.BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                css = """
                    ...
                    body { margin: 0; font-family: 'Outfit', sans-serif; background: linear-gradient(135deg, #0f172a 0%, #1e1b4b 100%); color: #f8fafc; min-height: 100vh; overflow-x: hidden; }
                    body::before { content:''; position:fixed; top:-50%; left:-50%; width:200%; height:200%; background: radial-gradient(circle at 50% 50%, rgba(99, 102, 241, 0.15) 0%, transparent 50%); z-index:-1; pointer-events:none; }
                    .glass { background: rgba(30, 41, 59, 0.7); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.1); box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3); }
                    .topbar { position: fixed; top: 0; left: 0; width: 100%; height: 70px; display: flex; align-items: center; padding: 0 30px; z-index: 100; border-bottom: 1px solid rgba(255, 255, 255, 0.05); box-sizing: border-box; }
                    .topbar h2 { font-weight: 800; background: linear-gradient(to right, #818cf8, #c084fc); -webkit-background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: -0.5px; }
                    .sidebar { position: fixed; top: 70px; width: 260px; height: calc(100% - 70px); padding: 30px 0; border-right: 1px solid rgba(255, 255, 255, 0.05); }
                    .sidebar a { display: flex; align-items: center; padding: 16px 30px; color: #94a3b8; text-decoration: none; font-weight: 600; transition: all 0.3s ease; border-left: 3px solid transparent; margin-bottom: 8px; }
                    .sidebar a:hover { color: #f8fafc; background: rgba(255, 255, 255, 0.05); padding-left: 35px; }
                    .sidebar a.active { color: #fff; background: rgba(99, 102, 241, 0.15); border-left-color: #818cf8; text-shadow: 0 0 10px rgba(129, 140, 248, 0.5); }
                    ...
                    .card { border-radius: 20px; padding: 35px; text-align: left; position: relative; overflow: hidden; }
                    .card::before { content:''; position:absolute; top:0; left:0; right:0; height:4px; background: linear-gradient(to right, rgba(99, 102, 241, 0.8), rgba(192, 132, 252, 0.8)); opacity: 0.5; }
                    .card h1 { margin-top: 0; font-weight: 800; letter-spacing: -1px; }
                    .card h3 { color: #cbd5e1; font-weight: 600; margin-top: 0; }
                    .console-box { background: rgba(0, 0, 0, 0.6); color: #a7f3d0; font-family: 'JetBrains Mono', monospace; padding: 20px; border-radius: 12px; height: 350px; overflow-y: auto; font-size: 13px; border: 1px inset rgba(255, 255, 255, 0.1); line-height: 1.5; box-shadow: inset 0 4px 20px rgba(0,0,0,0.5); scroll-behavior: smooth; }
                    .console-box::-webkit-scrollbar { width: 8px; } .console-box::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.2); border-radius:4px; }
                    .cmd-input { width: calc(100% - 100px); padding: 14px 20px; background: rgba(0, 0, 0, 0.4); border: 1px solid rgba(255,255,255,0.2); color: white; border-radius: 8px 0 0 8px; font-family: 'JetBrains Mono', monospace; outline: none; transition: 0.3s; }
                    .cmd-input:focus { border-color: #818cf8; background: rgba(0,0,0,0.6); }
                    .cmd-btn { width: 100px; padding: 14px; background: #6366f1; border: none; color: white; font-weight: 600; border-radius: 0 8px 8px 0; cursor: pointer; transition: 0.3s; }
                    .cmd-btn:hover { background: #4f46e5; }
                    .inv-grid { display: grid; grid-template-columns: repeat(9, 54px); gap: 4px; background: rgba(30,41,59,0.9); padding: 16px; border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; margin: 20px auto; width: fit-content; box-shadow: inset 0 4px 15px rgba(0,0,0,0.5); }
                    .slot { width: 54px; height: 54px; background: rgba(15,23,42,0.8); border: 1px solid rgba(255,255,255,0.05); border-radius: 4px; display: flex; align-items: center; justify-content: center; position: relative; transition: all 0.2s; }
                    .slot:hover { background: rgba(30,41,59,1); border-color: rgba(255,255,255,0.2); transform: scale(1.05); z-index: 30; }
                    .slot img { width: 34px; height: 34px; image-rendering: pixelated; z-index: 10; filter: drop-shadow(0 4px 6px rgba(0,0,0,0.4)); }
                    .hotbar-separator { grid-column: span 9; height: 12px; }
                    .player-item { display: flex; align-items: center; background: rgba(255,255,255,0.03); margin: 12px 0; padding: 15px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.05); transition: 0.3s; }
                    .player-item:hover { background: rgba(255,255,255,0.08); transform: translateY(-2px); }
                    .btn-action { padding: 8px 12px; border: none; color: white; border-radius: 6px; font-weight: 600; cursor: pointer; font-size: 12px; margin-left: 6px; transition: 0.3s; }
                    .btn-start { padding: 16px; background: linear-gradient(135deg, #10b981, #059669); border: none; color: white; border-radius: 10px; font-weight: 700; cursor: pointer; width: 100%; font-size: 16px; text-shadow: 0 1px 2px rgba(0,0,0,0.3); box-shadow: 0 4px 15px rgba(16, 185, 129, 0.4); transition: 0.3s; }
                    .btn-start:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(16, 185, 129, 0.6); }
                    .btn-stop { padding: 16px; background: linear-gradient(135deg, #ef4444, #dc2626); border: none; color: white; border-radius: 10px; font-weight: 700; cursor: pointer; width: 100%; font-size: 16px; text-shadow: 0 1px 2px rgba(0,0,0,0.3); box-shadow: 0 4px 15px rgba(239, 68, 68, 0.4); transition: 0.3s; }
                    .btn-stop:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(239, 68, 68, 0.6); }
                    .badge { position:absolute; bottom:2px; right:4px; font-size:14px; font-weight:800; color:white; font-family:'Outfit', sans-serif; text-shadow: 0 2px 4px rgba(0,0,0,1), 0 -1px 2px rgba(0,0,0,0.8); z-index:20; }
                """

                # PAGE: MAP (Logic Updated Here)
                if self.path == "/map":
                    ...
                    self.wfile.write(
                        f"<html><head><title>Live Map</title><meta charset='UTF-8'><style>{css}</style></head><body><div class='glass topbar'><h2>🪐 Antigravity Panel</h2></div><div class='glass sidebar'>{nav}</div><div class='main' style='padding:40px; display:block; height:calc(100vh - 70px);'>{content}</div></body></html>".encode()
                    )
                ...
                # PAGE: INVENTORY
                    ...
                    self.wfile.write(
                        f"<html><head><title>Inventory Viewer</title><meta charset='UTF-8'><style>{css}</style><meta http-equiv='refresh' content='5'></head><body><div class='glass topbar'><h2>🪐 Antigravity Panel</h2></div><div class='glass sidebar'>{nav}</div><div class='main'><div class='glass card' style='text-align:center;'><h2><img src='https://mc-heads.net/avatar/{player_name}/32' style='vertical-align:middle;border-radius:6px;margin-right:12px;'>{player_name}'s Items</h2><div class='inv-grid'>{inv_slots}</div><a href='/' style='color: #818cf8; text-decoration: none; font-weight:600;'>← Back to Dashboard</a></div></div></body></html>".encode()
                    )
                ...
                # PAGE: DASHBOARD
                    ...
                    self.wfile.write(
                        f"""<html><head><title>Admin Dashboard</title><meta charset="UTF-8"><style>{css}</style></head><body>
                        <div class="glass topbar"><h2>🪐 Antigravity Panel</h2></div>
                ...
                # PAGE: GAME RULES
                    ...
                    self.wfile.write(f"""<html><head><title>Game Rules</title><meta charset="UTF-8"><style>{css}</style></head><body>
                    <div class="glass topbar"><h2>🪐 Antigravity Panel</h2></div>
                    <div class="glass sidebar">{nav}</div>
                    <div class="main">
                    ...
                    self.wfile.write(f"""<html><head><title>Chat Relay</title><meta charset="UTF-8"><style>{css}</style><meta http-equiv="refresh" content="3"></head><body>
                        <div class="glass topbar"><h2>🪐 Antigravity Panel</h2></div>
                        <div class="glass sidebar"><a href="/">📊 Dashboard</a><a href="/map">🗺️ Map</a><a href="/chat" class="active">💬 Chat</a><a href="/inventory">🎒 Inventories</a><a href="/backups">💾 Backups</a><a href="/server_props">⚙️ Server Props</a><a href="/scheduler">⏰ Scheduler</a><a href="/settings">🛡️ Settings</a></div>
                ...
                # PAGE: BACKUPS
                    ...
                    self.wfile.write(f"""<html><head><title>World Backups</title><meta charset="UTF-8"><style>{css}</style></head><body>
                        <div class="glass topbar"><h2>🪐 Antigravity Panel</h2></div>
                        <div class="glass sidebar"><a href="/">📊 Dashboard</a><a href="/map">🗺️ Map</a><a href="/chat">💬 Chat</a><a href="/inventory">🎒 Inventories</a><a href="/backups" class="active">💾 Backups</a><a href="/server_props">⚙️ Server Props</a><a href="/scheduler">⏰ Scheduler</a><a href="/settings">🛡️ Settings</a></div>
                ...
                # PAGE: SERVER PROPERTIES EDITOR
                    ...
                    self.wfile.write(f"""<html><head><title>Server Properties</title><meta charset="UTF-8"><style>{css}</style></head><body>
                        <div class="glass topbar"><h2>🪐 Antigravity Panel</h2></div>
                        <div class="glass sidebar"><a href="/">📊 Dashboard</a><a href="/map">🗺️ Map</a><a href="/chat">💬 Chat</a><a href="/inventory">🎒 Inventories</a><a href="/backups">💾 Backups</a><a href="/server_props" class="active">⚙️ Server Props</a><a href="/scheduler">⏰ Scheduler</a><a href="/settings">🛡️ Settings</a></div>
                ...
                # PAGE: PLAYER STATS
                    ...
                    self.wfile.write(f"""<html><head><title>Player Stats</title><meta charset="UTF-8"><style>{css}</style></head><body>
                        <div class="glass topbar"><h2>🪐 Antigravity Panel</h2></div>
                        <div class="glass sidebar"><a href="/">📊 Dashboard</a><a href="/map">🗺️ Map</a><a href="/chat">💬 Chat</a><a href="/inventory">🎒 Inventories</a><a href="/backups">💾 Backups</a><a href="/server_props">⚙️ Server Props</a><a href="/scheduler">⏰ Scheduler</a><a href="/settings">🛡️ Settings</a></div>
                        <div class="main"><div class="glass card" style="grid-column:1/-1;">
                ...
                # PAGE: AUTO RESTART SCHEDULER
                    ...
                    self.wfile.write(f"""<html><head><title>Auto-Restart Scheduler</title><meta charset="UTF-8"><style>{css}</style></head><body>
                        <div class="glass topbar"><h2>🪐 Antigravity Panel</h2></div>
                        <div class="glass sidebar"><a href="/">📊 Dashboard</a><a href="/map">🗺️ Map</a><a href="/chat">💬 Chat</a><a href="/inventory">🎒 Inventories</a><a href="/backups">💾 Backups</a><a href="/server_props">⚙️ Server Props</a><a href="/scheduler" class="active">⏰ Scheduler</a><a href="/settings">🛡️ Settings</a></div>
                ...
                # 404 Fallback
                    ...
                    self.end_headers()
                    self.wfile.write(
                        f"<html><head><title>404</title><style>{css}</style></head><body>"
                        f"<div class='glass topbar'><h2>🪐 Antigravity Panel</h2></div>"
